...
class _GrowthSimulationState extends State<GrowthSimulation> {
      ...
      child: Padding(
        ...
        child:
            isLoading
                ...
                : data == null
                ? const Text(
                  "⚠️ Unable to load growth data from API.",
  ...
  Widget _buildSimulationContent() {
    ...
    if (!scenarios.containsKey(selectedScenario)) {
      return const Text("⚠️ Missing scenario data from API");
    }

    final currentScenarioData =
        (scenarios[selectedScenario] as List).cast<Map>();
    final spots =
        currentScenarioData
            .asMap()
            .entries
            .map(
              (e) => FlSpot(
                e.key.toDouble(),
                (e.value["revenue"] / 1000000).toDouble(),
              ),
            )
            .toList();
    ...
    final currentMetrics = (data!["current_metrics"] ?? {}) as Map;
            ...
            child: LineChart(
              LineChartData(
                ...
                titlesData: FlTitlesData(
                  ...
                  rightTitles: AxisTitles(
                    sideTitles: SideTitles(showTitles: false),
                  ), // Remove right axis heading
                  topTitles: AxisTitles(
                    sideTitles: SideTitles(showTitles: false),
                  ), // Remove top axis heading
          ...
          if (currentMetrics.isNotEmpty)
            Row(
              ...
              children: [
                _metricCard(
                  "ARR",
                  "\$${_formatNumber(currentMetrics["arr"] ?? 0)}",
                ),
                _metricCard(
                  "Growth",
                  "${currentMetrics["growth_rate"]?.toString() ?? '—'}%",
                ),
                _metricCard(
                  "Burn",
                  "\$${_formatNumber(currentMetrics["burn_rate"] ?? 0)}",
                ),
                _metricCard(
                  "Margin",
                  "${currentMetrics["gross_margin"]?.toString() ?? '—'}%",
                ),
              ],
            ),
          const SizedBox(height: 20),
  ...
  Widget _aiInsight(String scenario) {
    String insight = "";
    switch (scenario) {
      case "optimistic":
        insight =
            "Aggressive scaling with 150% YoY revenue growth. Profitability accelerates post-2027.";
        break;
      case "pessimistic":
        insight =
            "Cautious 50% YoY growth under stress. Focus on maintaining healthy burn margins.";
        break;
      default:
        insight = "Stable 100% YoY growth expected with sustainable margins.";
    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [kAccent1.withOpacity(0.07), kAccent2.withOpacity(0.07)],
        ),
        borderRadius: BorderRadius.circular(15),
      ),
      child: Text(
